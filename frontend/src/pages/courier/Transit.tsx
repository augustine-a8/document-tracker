...
export default function Transit() {
  const [transitMails, setTransitMails] = useState<IMail[]>([]);
  ...
  useEffect(() => {
    setApiLoading(true);
    getAllMailsApi("transit", start, limit, searchQuery, date)
      .then((res) => {
        if (res.status === 200) {
          setTransitMails(res.data.allTransitMails);
  ...
  return (
    <>
      ...
      {transitMails.length > 0 ? (
        ...
        <EmptyPage
          message={`No mails in transit for ${new Date(date).toDateString()}!`}
        />
      )}
    </>
  );
